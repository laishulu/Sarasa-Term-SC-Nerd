...
- 提供了 `Sarasa Term SC Nerd` 和 `Sarasa Term SC Widenerd` 两种字体，二者的区别
  仅在于那些可横向拉伸的图标在前者占一个英文字符宽，在后者占两个英文字符宽。
- 一些符号进行了纵向拉伸，不会出现`Powerline`条带中高低不一，无法上下对齐的情况。
- 原始`Sarasa Term SC`字体、`Sarasa Term SC Nerd`和`Sarasa Term SC Widenerd`字体
  可以共存，不会产生冲突。
...
在你的主题配置文件中，使用 `Sarasa Term SC Nerd` 或者 `Sarasa Term SC Nerd` 。
